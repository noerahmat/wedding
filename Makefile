...
.PHONY: shell help build rebuild service login test clean prune develop push
...
service:
	...
	# docker pull mamat08nurahmat/wedding
	docker run -d -p 80:80 --restart=always --name $(PROJECT_NAME) mamat08nurahmat/wedding

develop:
	# run as a (background) service
	# docker-compose -p $(PROJECT_NAME)_$(HOST_UID) up -d $(SERVICE_TARGET)
	# docker run -d -p 82:80 -v /home/ekrut-48/Downloads/hookup:/usr/share/nginx/html --name develop-$(PROJECT_NAME) mamat08nurahmat/wedding
	docker run -d -p 81:80 -v ${PWD}:/usr/share/nginx/html --name develop1-$(PROJECT_NAME) mamat08nurahmat/wedding

push:
	docker push mamat08nurahmat/wedding
...
build:
	...
	docker build -t mamat08nurahmat/wedding -f docker/Dockerfile .
...
clean:
	...
	docker stop --rmi all 2>/dev/null \
